...
/**
 * ملف النظام والتهيئة الأساسية للموقع - الإصدار الشامل والمصحح نهائياً
 */
const CONFIG = {
    WHATSAPP_NUMBER: "967779087415",
    DEFAULT_ADMIN_PASS: "1234",
...
class Store {
    static getKnowledge() {
        try {
            ...
            // دمج دقيق بدون تكرار للشهادات المرفوعة
            const uniqueCertsMap = new Map();
            [...baseCerts, ...visionCerts].forEach(c => uniqueCertsMap.set(c.id || c.title, c));
...
const App = {
    ...
    /**
     * دالة الحذف الشاملة والنهائية (تحذف من جميع التخزينات وتأمر الموقع بإزالة كافة البيانات عنه)
     */
    deleteItem(key, index) {
        ...
        if (db[key] && Array.isArray(db[key])) {
            ...
            // 1. الحذف من قاعدة المعرفة الأساسية
            db[key].splice(index, 1);
            Store.saveKnowledge(db);
            
            // 2. الحذف الشامل من التخزين المرتبط بالمستندات المرفوعة (my_certs)
            if (key === 'certificates' && itemToDelete) {
                try {
                    let savedCerts = JSON.parse(localStorage.getItem('my_certs') || '[]');
                    // تصفية وحذف المطابق بالاسم أو الإزالة بالتوافق
                    savedCerts = savedCerts.filter(c => c.title !== itemToDelete.title);
                    localStorage.setItem('my_certs', JSON.stringify(savedCerts));
                } catch(e) {}
            }

            // 3. إعادة تحديث قوائم الإدارة والواجهة الرئيسية فوراً
            this.renderAdminLists(db);
            this.renderAll();
            renderCertifications();
    ...
    async sendChatMessage() {
        ...
        // 🛡️ نظام الرد الذكي المحلي البديل في حال تعطل الاتصال أو فراغ المفاتيح لكي لا يتوقف المساعد أبدًا
        if (!success) {
