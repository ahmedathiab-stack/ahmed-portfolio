...
/**
 * الكائن الرئيسي لتشغيل وعرض محتوى الموقع ووظائفه - الإصدار المدمج (بدون تكرار)
 */
const App = {
    ...
    // دالة موحدة لحفظ أو تعديل الشهادة (تدمج الرفع من الجهاز أو الرابط بشكل كامل)
    saveCertificate() {
        ...
        const imageUrlInput = document.getElementById('certImage').value.trim();
    ...
    async sendChatMessage() {
        ...
        const strictSystemPrompt = `You are the personal assistant of Trainer Ahmed Adel Naji Thiab. KNOWLEDGE BASE: ${JSON.stringify(kb)}`;
        ...
        if (!success) {
            msgContainer.innerHTML += `<div class="msg bot-msg">أهلاً بك! يمكنك مراسلة الأستاذ أحمد عادل مباشرة عبر الواتساب: +967${CONFIG.WHATSAPP_NUMBER}</div>`;
        }
        msgContainer.scrollTop = msgContainer.scrollHeight;
    }
};
...
function renderCertifications() {
    ...
    certContainer.innerHTML = (db.certificates || []).map((cert, idx) => {
        const certId = cert.id || `cert-${idx}`;
        ...
        return `
            <div class="cert-item ${isUnlocked ? 'unlocked' : ''}">
                ...
                <div class="cert-info">
                    <h4>${cert.title}</h4>
                    <p>📌 ${cert.issuer} | <span style="color:var(--primary-color)">${cert.category || 'عام'}</span></p>
                </div>
                <div>
                    ${isUnlocked ? 
